...
- 왜 '행 삭제'가 아니라 '토큰 제거'인가:
  닉네임이 든 문장은 대개 "기우자 대가리로 올라가" 같은 **게임 오더(clean)** 다. 행을 통째로 지우면
  멀쩡한 clean 게임챗 예시를 대량 버리게 된다 → 이름만 빼고 문장은 살려 **데이터를 보존**한다.
  → "기우자 대가리로 올라가" → "대가리로 올라가" (privacy 해결 + `[유저]` 토큰도 안 씀 + clean 예시 보존)
  ※ 이런 게임 상황 발화는 base unSmile이 가끔 욕설로 오인(기획서의 "죽어 죽어" 류 과탐)하기도 해
    보존하면 과탐 감소에도 도움 — 단 **튜닝의 주 목적은 게임 욕설 Recall 개선**이고 과탐 감소는 부차다.
