...
- **선정 기준**: 임계값과 무관한 **AP·AUROC**(부정어 이진 랭킹)가 주력, LRAP는 참고(이 test_set은 활성 라벨 1개라 사실상 이진). 0.5 한 점의 F1에 의존하지 않음
...
> 숫자 읽는 법: baseline의 **Recall/F1/Precision**은 Step 2와 같은 `not-clean > 0.5` 운영점 성능입니다. **AP 91.94**는 Step 1 후보 선정 때 쓴 이진 abuse 랭킹 지표이고, **LRAP 0.887**은 10개 라벨 순위 기반 지표지만 이 test_set은 활성 라벨 1개라 사실상 이진이라, 서로 같은 점수가 아닙니다.
