...
- **첫 응답 지연**(발화 시작→첫 중간결과 = 스트리밍 응답성) · **평균 문장 간격**(확정 문장 사이) · 인식 단어 수
- 욕설이 **검열 없이 원문 인식**되는지 확인 (욕설탐지 입력으로 적합한지)

> ⏱️ "첫 응답 지연"은 **첫 중간(interim) 결과**까지의 시간이라 보통 수백 ms다. 확정(final)은 엔진이 말끝 침묵을 감지해야 떨어져서 더 늦는데(발화 길이 포함), 그건 "응답 지연"이 아니라 "발화+확정 시간"이라 따로 본다.
