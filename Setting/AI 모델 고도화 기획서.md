...
- **Baseline 악플/욕설 Recall: 0.59** (개선 필요!)
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Step 0: Baseline 테스트 (현재 모델 성능 파악)                   │
│          - Keywords 자주 쓰는 말로 수정 + GPT 보강               │
│          - 기존 unSmile 모델에 테스트                            │
│          - 인식률 떨어지는 표현 목록화 (시각화)                  │
│                    │                                             │
│                    ▼                                             │
│  Step 1: 게임 STT 데이터 500건 수집 + 라벨링                     │
│                    │                                             │
│                    ▼                                             │
│  Step 2: unSmile 데이터셋 보정                                   │
│          - 인식률 떨어지는 표현 수정                             │
│          - 예: "빡친다" clean → 욕설로 재라벨링                  │
│                    │                                             │
│                    ▼                                             │
│  ┌─────────────────┴─────────────────┐                          │
│  │                                   │                          │
│  ▼                                   ▼                          │
│  Step 3-1: LoRA            Step 3-2: Full Fine-tuning           │
│  Fine-tuning               (전체 가중치 학습)                    │
│  (어댑터만 학습)           (보정된 unSmile + 500건)              │
│  │                                   │                          │
│  └─────────────────┬─────────────────┘                          │
│                    │                                             │
│                    ▼                                             │
│  Step 4: 성능 비교 분석 (LoRA vs Full FT)                        │
...
│                    │                                             │
│                    ▼                                             │
│  Step 6: INT8 양자화 (CPU 추론 최적화)                           │
│                    │                                             │
│                    ▼                                             │
│  Step 7: AI 서버 (FastAPI)에 이식/배포                           │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| 단계 | 작업 | 상세 내용 | 예상 시간 | 도구 |
|------|------|-----------|-----------|------|
| **0** | Keywords 준비 | 자주 쓰는 말로 수정 + GPT 보강 | 1시간 | GPT |
| **1** | Baseline 테스트 | 기존 unSmile에 keywords 테스트, 인식률 떨어지는 목록 시각화 | 1시간 | Colab |
| **2** | 게임 STT 데이터 라벨링 | 500건 라벨링 (악플/욕설, clean) | 2-3시간 | 수동 |
| **3-1** | LoRA Fine-tuning | 500개 데이터셋으로 LoRA FT | 20-30분 | Colab GPU |
| **3-2** | unSmile 데이터 수정 | 인식률 떨어지는 표현 재라벨링 (예: 빡친다 clean→욕설) | 30분 | Python |
| **3-2** | Full Fine-tuning | 보정된 unSmile + 500건으로 Full FT | 30-40분 | Colab GPU |
| **4** | 모델 비교 | LoRA vs Full FT 성능 비교 | 10분 | Colab |
| **5** | 양자화 | INT8 Dynamic Quantization | 10분 | Colab |
| **6** | 최종 모델 완성 | 최적 모델 선택 및 저장 | 5분 | Colab |
| **7** | 이식 | FastAPI 서버에 배포 | 30분 | EC2 |
