...
| **test_set.tsv** | **740** | abuse 228 / clean 512 | **held-out 테스트셋** — 모델 선정·비교·양자화 평가에 공통 사용 |
...
- **`[유저]` 익명화 토큰 제거**: 학습 데이터·실제 추론엔 `[유저]`가 없으므로, 테스트의 `[유저]` 54건을 제거해 train·운영과 형식 일치 (제거 후 중복 2건도 삭제 → 742 → **740**)
...
```
> ⚠️ 테스트는 항상 `test_set.tsv`(740) 하나로 통일 — 단계마다 다른 셋을 쓰지 말 것.
