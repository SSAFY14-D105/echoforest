...
| **test_set.tsv** | **742** | abuse 229 / clean 513 | **held-out 테스트셋** — 모델 선정·비교·양자화 평가에 공통 사용 |

### test_set.tsv 무결성 (검증됨)
- 공식 unSmile(원본·보정, train·valid)과 **0 겹침**
- `train_collected.tsv`(518)와도 **0 겹침** → 8개 모델(v1·v2) 누구를 평가해도 누수 없음
- 구성: 기존 게임 벤치 187 + 사람이 라벨링한 게임 STT(human_labeled의 라벨분), 중복·학습누수 제거 후 742
...
```
> ⚠️ 테스트는 항상 `test_set.tsv`(742) 하나로 통일 — 단계마다 다른 셋을 쓰지 말 것.
