"""
6_Model_Comparison — 9개 모델 비교 (baseline + 파인튜닝 8) on test_set(482)
=============================================================================
- abuse 판정 = 악플/욕설(index 8) > 0.5 — 배포·Step 1·Step 2와 동일 정의
- 지표: Abuse/Clean Precision·Recall·F1 + LRAP
- 산출물: results/ (comparison_results.csv/.json) → 차트는 plot_comparison.py
...
DEVICE = torch.device("cpu")   # 9모델×482, CPU로 안정적
MAX_LEN = 128
...
print(f"test_set: {len(test)}  abuse(idx8): {int(labels[:,8].sum())}  clean: {int(labels[:,9].sum())}")
...
def evaluate(name, path, tok_key):
    ...
    ap, ar, af, _ = precision_recall_fscore_support(labels[:,8], preds[:,8], average="binary", zero_division=0)
    cp, cr, cf, _ = precision_recall_fscore_support(labels[:,9], preds[:,9], average="binary", zero_division=0)
    lrap = label_ranking_average_precision_score(labels, probs)
    tn, fp, fn, tp = confusion_matrix(labels[:,8], preds[:,8]).ravel()
    del model
...
for name, (path, tok_key) in MODELS.items():
    ...
    try:
        ...
        print(f"  ✓ Abuse R={r['abuse_recall']:.4f} F1={r['abuse_f1']:.4f} P={r['abuse_precision']:.4f} LRAP={r['lrap']:.4f}")
...
json.dump({"test_n": len(test), "abuse_definition": "악플/욕설(index8)>0.5", "results": rows},
...
print(f"💾 results/comparison_results.csv·.json")
